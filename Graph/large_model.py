...
class Conv(nn.Module):
    def __init__(self, hidden_dim, nheads, trans_dropout, feat_dropout, adj_dropout):
        ...
        self.preffn_dropout = nn.Dropout(feat_dropout)
        self.x_ffn_dropout = nn.Dropout(feat_dropout)
    ...
    def forward(self, eig, u, ut, graph, x_feat, edge_attr, eig_mask, edge_idx):
        B, N = eig.size()[:2]

        mha_eig = self.mha_norm(eig)
        mha_eig, attn = self.mha(mha_eig, mha_eig, mha_eig, key_padding_mask=eig_mask)
        ...
        new_e = self.decoder(eig).transpose(1, 2)    # [B, m, N]
        diag_e = torch.diag_embed(new_e)             # [B, m, N, N]

        identity = torch.diag_embed(torch.ones(B, N)).to(u.device)
        bases = [identity]
        ...
        bases = edge_softmax(graph, bases)
